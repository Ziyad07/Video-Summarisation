...
#        self.hog_c3d_features_test.sort()
    ...
    def prepare_train_data(self):
        ...
        for i in range(len(self.feature_vector_names)): # 80% for training
            ...
            targetss = self.shrinkTargets(targets)       
            print targetss.shape
...
#            np.save('../../saved_numpy_arrays/TvSum50/c3d_hog/'+ fileInProgress + '.npy', c3d_hog_feature_matrix)
    ...
    def shrinkTargets(self, videoData_targets):
#        targets = np.zeros((1,))
        averageList=[]
        partitions = videoData_targets.shape[0]/16
        frames = partitions * 16
        for j in xrange(0, frames, 16):
            average = 0
            for k in range(16):
                average = average + videoData_targets[j+k]
